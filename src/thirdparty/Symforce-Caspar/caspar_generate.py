...
def spherical_core(
    ...
    """Reprojection residual for COLMAP's SPHERICAL model (sensor/models.h).

    Equirectangular projection. (w, h) are the image dimensions, held constant
    in BA (the model has no focal length, principal point, or distortion), so
    only pose and point are tunable. Mirrors SphericalCameraModel::ImgFromCam:
