...
class CasparBundleAdjuster : public BundleAdjuster {
 ...
 private:
  ...
  CasparSolverSizing BuildSizing() const {
    ...
    if (auto it = num_poses_per_model_.find(CameraModelId::kSpherical);
        it != num_poses_per_model_.end()) {
    ...
    if (const ModelData* md = get_md(CameraModelId::kSpherical)) {
      // SPHERICAL has no calib pool; FillSizing only reads the factor counts.
      adapters_.at(CameraModelId::kSpherical)
          ->FillSizing(sz, *md, get_n(CameraModelId::kSpherical));
    }
    return sz;
  }
